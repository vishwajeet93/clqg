...
class DataUtil(object):
    ...
    def get_training_batches(self,
        ...
        #Need to edit the shuffle for unison shuffling
        if shuffle:
            #src_shuf_path, dst_shuf_path = self.shuffle([src_path, dst_path])
            #src_shuf_path, dst_shuf_path, src_1_shuf_path, dst_1_shuf_path = self.shuffle([src_path, dst_path, self.config.train.src_path_1, self.config.train.dst_path_1])
            if self.config.train.src_more:
              dst_shuf_path, dst_1_shuf_path, src_shuf_path, src_1_shuf_path = self.shuffle([dst_path, self.config.train.dst_path_1, src_path, self.config.train.src_path_1])
            else:
              src_shuf_path, dst_shuf_path, src_1_shuf_path, dst_1_shuf_path = self.shuffle([src_path, dst_path, self.config.train.src_path_1, self.config.train.dst_path_1])
            ...
            src_1_shuf_path = self.config.train.src_path_1
            dst_1_shuf_path = self.config.train.dst_path_1

        src_sents, dst_sents = [], []
        src_1_sents, dst_1_sents = [], []
        for src_sent, src_1_sent, dst_sent, dst_1_sent in izip(codecs.open(src_shuf_path, 'r', 'utf8',errors='ignore'), codecs.open(src_1_shuf_path, 'r', 'utf-8', errors='ignore'), 
                                       codecs.open(dst_shuf_path, 'r', 'utf8', errors='ignore'), codecs.open(dst_1_shuf_path, 'r', 'utf-8', errors='ignore')):
            # If exceed the max length, abandon this sentence pair.
            src_sent = src_sent.split()
            dst_sent = dst_sent.split()
            src_1_sent = src_1_sent.split()
            dst_1_sent = dst_1_sent.split()
            if  len(src_sent) > max_length or len(dst_sent) > max_length or len(src_1_sent) > max_length or len(dst_1_sent) > max_length:
                continue
            src_sents.append(src_sent)
            dst_sents.append(dst_sent)
            src_1_sents.append(src_1_sent)
            dst_1_sents.append(dst_1_sent)
            # Create a padded batch.
            if len(src_sents) >= batch_size:
                yield self.create_batch(src_sents, o='src'), self.create_batch(dst_sents, o='dst'), self.create_batch(src_1_sents, o='src'), self.create_batch(dst_1_sents, o='dst')  
                src_sents, dst_sents = [], []
                src_1_sents, dst_1_sents = [], []
    ...
    def get_training_batches_with_buckets(self, shuffle=True):
        ...
        for bucket in buckets:
            ...
            #print("len", self.config.train.devices.split(','))
            #print("bucket", len(caches[bucket][0]))
    ...
    def get_test_batches(self,
        ...
        if set_src_path and set_batch:
            ...
            #src_path = self.config.test.dst_path
